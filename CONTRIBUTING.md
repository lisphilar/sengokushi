...
- CE_0310: プレーヤーが国、特定の家の直轄拠点を選択する
  - 引数`Jyoshu`: 拠点の城主（直轄）
  - 返り値`ShiroSelected`: 選択した直轄拠点の城番号
...
- CE_0319: 特定の城について、史実上・石高上、現城主が新城主に手放すことを受け入れられるかどうかを評価する
...
- CE_0067: 家情報のメッセージ表示（一般大名家、家臣、牢人の区分によってテンプレートは異なる）
  - 引数`Target`: 対象となる家の番号
- CE_0305: プレーヤーが生存独立大名家（プレーヤーの主家を除く）を選択する
...
- CE_0131: プレーヤーが主家として配下を決定し、加増を実施する
  - 返り値`DaimyouDetermined`: 加増を受けた家の大名番号
  - 返り値`ShiroDetermined`: 加増の対象となった主家側の直轄拠点の城番号
- CE_0224: 特定の大名家が主家として、特定の配下に対して加増を実施する
  - 引数`SyukeDetermined`: 主家側の大名番号
  - 引数`DaimyouDetermined`: 加増を受けた家の大名番号
  - 返り値`ShiroDetermined`: 加増の対象となった主家側の直轄拠点の城番号
- CE_0281: 特定の大名家が主家として、特定の配下に対して特定の拠点の加増を実施する
  - 引数`SyukeDetermined`: 主家側の大名番号
  - 引数`DaimyouDetermined`: 加増を受けた家の大名番号
  - 引数`ShiroDetermined`: 加増の対象となる主家側の直轄拠点の城番号
- CE_0056: プレーヤーが主家として配下を決定し、特定の国全体について加増またはその約束を実施する
  - 返り値`DaimyouDetermined`: 加増を受けた家の大名番号
  - 返り値`ShiroDetermined`: 加増の対象となった主家側の直轄拠点の城番号
