...
- CE_0158: 経済を「%a%,%b%%c%%d%」の形式に変換する。
  - 引数`Value`: 経済の数値
  - 返り値`a`: 千以上, `b`: 百の位, `c`: 十の位, `d`: 一の位
- CE_0215: 大名家の借金算出
  - 引数`Target`: 大名家の番号
  - 返り値`Debt`: 借金の額
...
- CE_0105: プレーヤーが指定した国内に大名家の特定
  - 返り値`DShiroNCountryList[大名番号]`: 各大名家の、指定した国内にある直轄拠点の数
- CE_0153: 所在地や領主、石高、経済など城の情報をメッセージにて表示
  - 引数、返り値についてはCE_0183と同じ
...
- CE_0067: 家情報の表示（一般大名家、家臣、牢人の区分によってテンプレートは異なる）
- CE_0125: 牢人家の一覧をメッセージにて表示
  - 返り値`RouninCount`: 牢人家の最大数（=牢人用の拠点の数）
  - 返り値`DRouninList[カウント]`: 各牢人家の大名番号（0$\leq$大名番号$<$`RouninCount`）
  - 返り値`SNameForRouninList[カウント]`: 各牢人家の所有する拠点の城番号（0$\leq$大名番号$<$`RouninCount`）
- CE_0133: 主家（プレーヤーを除く）検索後、従属家・臣従家（家臣以外）・家臣をプレーヤーが選択
  - 返り値`DaimyouDetermined`: 選択された家の大名番号
- CE_0134: 主家（プレーヤーを含む）検索後、従属家・臣従家（家臣以外）・家臣をプレーヤーが選択
...
- CE_0131: プレーヤーが主家として配下を決定し、増封を実施する
  - 返り値`DaimyouDetermined`: 増封を受けた家の大名番号
  - 返り値`ShiroDetermined`: 増封の対象となった主家側の直轄拠点の城番号
- CE_0224: 特定の大名家が主家として、特定の配下に対して増封を実施する
  - 引数`SyukeDetermined`: 主家側の大名番号
  - 引数`DaimyouDetermined`: 増封を受けた家の大名番号
  - 返り値`ShiroDetermined`: 増封の対象となった主家側の直轄拠点の城番号
- CE_0281: 特定の大名家が主家として、特定の配下に対して特定の拠点の増封を実施する
  - 引数`SyukeDetermined`: 主家側の大名番号
  - 引数`DaimyouDetermined`: 増封を受けた家の大名番号
  - 引数`ShiroDetermined`: 増封の対象となる主家側の直轄拠点の城番号
- CE_0131: プレーヤーが主家として配下を決定し、特定の国全体について増封またはその約束を実施する
  - 返り値`DaimyouDetermined`: 増封を受けた家の大名番号
  - 返り値`ShiroDetermined`: 増封の対象となった主家側の直轄拠点の城番号
