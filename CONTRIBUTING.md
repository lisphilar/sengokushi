...
- CE_0358: 配列の値が降順となるようにインデックスを並び替える
  ...
  - 引数`IndexForIgnored`：インデックスに対する値が`IgnoredValue`に一致した場合の`RankL[順位]`の値（`NA`のときインデックス自身）
...
- CE_0155: 特定の家との関係性を一覧化する
  ...
  - 返り値：`JyuzokuK`: `Target`に従属する一般大名家の石高合計
  - 返り値：`ShinjyuK`: `Target`に臣従する一般大名家の石高合計
  - 返り値：`KashinK`: `Target`の家臣の石高合計
- CE_0067: 家情報のメッセージ表示（一般大名家、家臣、牢人の区分によってテンプレートは異なる）
  - 引数`Target`: 対象となる家の番号
    - `NA`の場合：プレーヤーが独立大名家を選択 $\to$ 情報表示 $\to$ 配下より`Target`を選択
...
- CE_0201: プレーヤーが、生存独立大名家を選択する
  - 返り値`DIndependent`：選択された生存独立大名家の大名番号
